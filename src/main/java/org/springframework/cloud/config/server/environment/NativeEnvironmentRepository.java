...
public class NativeEnvironmentRepository implements EnvironmentRepository, SearchPathLocator, Ordered {
    ...
    public Environment findOne(String config, String profile, String label) {
        Environment environment = getEnvironment(config, profile, label);
        Environment environmentBase = getEnvironment("base", "base", label);
        List<PropertySource> sources = environment.getPropertySources();
        List<PropertySource> baseSources = environmentBase.getPropertySources();
        sources.addAll(baseSources);
        return environment;
    }

    /**
     *  重写该逻辑，实现读取请求配置文件+base文件的合集
     *
     * @param config
     * @param profile
     * @param label
     * @return
     */
    private Environment getEnvironment(String config, String profile, String label) {
    ...
    private String[] getArgs(String application, String profile, String label) {
        ...
        list.add("--spring.config.location=" + StringUtils
                .arrayToCommaDelimitedString(getLocations(application, profile, label).getLocations()));
    ...
    private boolean isDirectory(String location) {
        return !location.contains("{") && !location.endsWith(".properties") && !location.endsWith(".yml") && !location
                .endsWith(".yaml");
    }
